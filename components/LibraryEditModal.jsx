...
export default function LibraryEditModal({ open, item, onClose }) {
  ...
  useEffect(() => {
    if (!open) return;
    const base = item || {};
    const actorsArr = Array.isArray(base.actors)
      ? base.actors
      : (typeof base.actors === 'string' && base.actors.trim() ? [base.actors] : []);
    const commentsArr = Array.isArray(base.comments)
      ? base.comments
      : (typeof base.comments === 'string' && base.comments.trim() ? [base.comments] : []);
    setForm({
      title: base.title || '',
      ...
      year: base.year != null ? String(base.year) : '',
      price: base.price != null ? String(base.price) : '',
      format: base.format || '',
      link: base.link || '',
      actors: actorsArr.length ? actorsArr.map(a => String(a || '')) : [''],
      comments: commentsArr.length ? commentsArr.map(c => String(c || '')) : [''],
    });
  ...
  const onSave = () => {
    ...
    updateLibraryItem(item.id, {
      ...
      year: (normalized.type === 'movie' || normalized.type === 'game') ? normalized.year : '',
      price: normalized.type === 'purchase' ? normalized.price : '',
      format: normalized.type === 'comic' ? normalized.format : '',
      link: (normalized.link || '').trim(),
      actors: (normalized.type === 'movie' || normalized.type === 'series') ? normalized.actors : [],
      comments: normalized.type === 'comic' ? normalized.comments : [],
    });
    onClose?.();
  };
